...
sequenceDiagram
    ...
    U->>B: Nhap email + password
    B->>A: POST /login
    A->>A: Verify password
    alt MFA not required
        A->>S: Create full session
        A-->>B: Logged in
    else MFA required
        A->>M: Create short challenge
        A-->>B: pending_mfa token
        U->>B: Submit TOTP / passkey
        B->>A: POST /mfa/verify
        A->>M: Verify challenge + factor
        A->>S: Rotate to full session
        A-->>B: Logged in
    end
```
...
```mermaid
flowchart LR
    Secret[Shared secret] --> App[Authenticator app]
    Secret --> Server[Server]
    Time[30s time window] --> App
    Time --> Server
    App --> Code[6-digit code]
    Code --> Server
```
...
sequenceDiagram
    ...
    B->>S: GET registration options
    S-->>B: challenge + rpID
    B->>K: navigator.credentials.create()
    K-->>B: attestation
    B->>S: Verify registration
...
sequenceDiagram
    ...
    B->>S: GET auth options
    S-->>B: challenge
    B->>K: navigator.credentials.get()
    K-->>B: signed assertion
    B->>S: Verify signature
    S->>S: Rotate session
```
